...
<jsp:include page="/app/common/head.jsp">
    <jsp:param name="title" value="基金管理"/>
</jsp:include>
            ...
            onelist(page,keywords,'',function(data){
                ...
                if(data.data.length>0){
                    for(var i=0;i<data.data.length;i++){
                     ...
                     li = '<li><a href=${pageContext.request.contextPath}/app/getFundDetail.action?userid=${userid}&&pkid='+data.data[i].pkid+'><dl><dt>' + data.data[i].qiyename+ '<span class="fr">'+data.data[i].chstatus+'</span></dt><dd><span>公司性质：</span>' + qiyequality + '</dd><dd><span>法定代理人：</span>' + data.data[i].corp + '</dd><dd><span>成立日期：</span>' + data.data[i].regdate + '</dd><dd><span>录入时间：</span>' + data.data[i].lurushijian + '</dd></dl></a></li>'
                        $('.list').append(li);
                    };
                }
            });
