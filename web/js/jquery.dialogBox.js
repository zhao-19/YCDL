 ...
 */
 ...
 ;(function($,window,document){
 	...
 	DialogBox.prototype = {	
 		...
 		//显示弹出框
 		show: function(){
 			$('.dialog-box').show(10);//有了这个盒子，有的时候就需要点击两次才会使点击事件生效。

 			setTimeout(function(){
 				$('.dialog-box').addClass('show');
 			},10)

 			$('#dialog-box-mask').show(50);
 		},
 		...
 		setStyle: function(){
 			...
 			$dialog.css({
 				...
 				'margin-top': function(){
 					...
 					return -Math.round(height/2-102) + 'px';
 				},
 		...
 		trigger: function(element,event){
 			...
 			if($.isFunction(that.settings.confirm)){
 				$('.dialog-btn-confirm').on('click',function(){
 					...
 					//增加自己的实现
 	 			 	$.ajax({
 	 		            url:'/incre/submit.html',
 	 		            type:'POST',
 	 		            data:{
 	 			 			increid	:$("#increid").val(),
 	 			 			incename:$("#incename").val()
 	 		            },
 	 		            dataType:"json",
 			...
 			if($.isFunction(that.settings.cancel)){
 				$('.dialog-btn-cancel,.dialog-box-close').on('click',function(){
