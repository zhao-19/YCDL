...
dsy.add("0",["四川省","天津市","上海市","重庆市","河北省","山西省","内蒙古","辽宁省","吉林省","黑龙江省","江苏省","浙江省","安徽省","福建省","江西省","山东省","河南省","湖北省","湖南省","广东省","广西","海南省","北京市","贵州省","云南省","西藏","陕西省","甘肃省","青海省","宁夏","新疆","香港","澳门","台湾省"]);
dsy.add("0_0_0",["高新区","青羊区","锦江区","金牛区","武侯区","成华区","龙泉驿区","青白江区","新都区","温江区","都江堰市","彭州市","邛崃市","崇州市","金堂县","赵镇","双流县","郫县","郫筒镇","大邑县","晋原镇","蒲江县","鹤山镇","新津县","五津镇"]);
...
dsy.add("0_22_0",["东城区","西城区","崇文区","宣武区","朝阳区","丰台区","石景山区","海淀区","门头沟区","房山区","通州区","顺义区","昌平区","大兴区","怀柔区","平谷区","密云县","延庆县","延庆镇"]);
...
var s=["gszcd_province","gszcd_city","gszcd_county"];//三个select的id

if(custtype=='aaaaaaaaaaaaaaaaaaaaaaaaaaaaa001') { //企业
	 s=["gszcd_province","gszcd_city","gszcd_county"];//三个select的id

}else if(custtype=='aaaaaaaaaaaaaaaaaaaaaaaaaaaaa002') {//个人
	 s=["person_province","person_city","person_county"];//三个select的id

}else if(custtype=='aaaaaaaaaaaaaaaaaaaaaaaaaaaaa003') {//机构
	 s=["jigou_province","jigou_city","jigou_county"];//三个select的id

}else if(custtype=='productczdfirst'){ //成长贷的三个字段
	 s=["gszcd_province","gszcd_city","gszcd_county"];//三个select的id
}
...
function _init_area(){  //初始化函数
		...
		if(custtype=='productczdfirst') {
		//机构
		var pro = $("#province").val();
		var pros = $("#gszcd_province option");
		pros.each(function(){
			if($(this).val()==pro){
				$(this).attr("selected",true);
				$("#gszcd_province").change();
			}
		});
		
		var cit = $("#city").val();
		var cits = $("#gszcd_city option");
		cits.each(function(){
			if($(this).val()==cit){
				$(this).attr("selected",true);
				$("#gszcd_city option").change();
			}
		});
		
		var cou = $("#county").val();
		var cous = $("#gszcd_county option");
		cous.each(function(){
			if($(this).val()==cou){
				$(this).attr("selected",true);
				//$("#gszcd_county option").change();
			}
		});
			/*if(pro.length<1){
				opt0[0]= "-请选择-";
