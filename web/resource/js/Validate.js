...
var validatePrompt = {
    username:{
        ...
        isNull:"请输入用户名",
        error:{
            beUsed:"该用户名已被使用，请使用其它用户名注册，如果您是&quot;{1}&quot;，请直接登录</a>",
        ...
        if (!length && format) {
            ...
            if (!namestate || nameold != option.value) {
                if (nameold != option.value) {
                	...
                	$.ajax({
                		  type: 'POST',
                		  url: "/user/checkAccountExist.html?e.account=" + escape(option.value) ,
                		  data: {},
            ...
            if (!format2) {
                ...
                if (!emailstate || emailold != option.value) {
                    if (emailold != option.value) {
                        ...
                        $.ajax({
                  		  type: 'POST',
                  		  url: "/user/checkEmailExist.html?e.email=" + escape(option.value) ,
                  		  data: {},
                  		  success: function(data){
                  			  if(data.flag==0)  {
                  				validateSettings.succeed.run(option);
