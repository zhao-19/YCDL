...
<script type="text/javascript">
    ...
    $(window).load(function () {
        ...
        if (backnotice == 'success') {
            if(backnoticebzf=='1'){
	            ggts2("数据保存成功！");
	            setTimeout(function(){
	    			window.location="/declare_yggqjl/selectAllList.html?type=yggqbt&init=y";
	   			},1500);
		    }
		    if(backnoticebzf=='2'){
		    	ggts2("数据提交成功！");
 ...
 function checkgbs(){
	...
	var zgbs = $("#cgptcg");
	//直接持股数(股)
	var cgptzgbs = $("#cgptzgbs");
	if((cgptzgbs.val()*1)>(zgbs.val()*1)){
		cgptzgbs.parent().children(".error").html("直接持股数不能大于“总股本数”！").show();
		bgbzf = false;
	}else{
		bgbzf = true;
	}
	...
	//公司股本
	var gsgbs = $("#dkk").children().find(".gsgb");
		gsgbs.each(function(){
		var gsgbv = $(this).val();
		if((gsgbv*1+$("#cgptzgbs").val()*1)>$("#cgptcg").val()){
			$(this).parent().children(".error").html("公司股本数+直接持股数不能大于“总股本数”！").show();
			bgbzf = false;
		}else{
			bgbzf = true;
		}
	...
	//员工股本数不能大于“平台股本数”！
	var yggbs = $("#dkk").children().find(".yggb");
		yggbs.each(function(){
		var ptgbv = $(this).parent().parent().prev().children().find('.ptgb').eq(0).val()*1;
		var yggbv = $(this).val()*1;
		if(yggbv>ptgbv){
			$(this).parent().children(".error").html("员工股本数不能大于“平台股本数”！").show();
			bgbzf2 = false;
		}else{
			bgbzf2 = true;
		}
    ...
    <div class="list fr">
        <div class="rightbox">
            ...
            <div id="zaiquanxinxi-update" class="vipinfo">
                ...
                <form id="form1" action="/declare_yggqjl/saveData.html" method="post">
                    ...
                    <div class="inputtest inputtestl">
                        <span class="leftname leftnamel">股权激励完成时间：<span style="color:red">*</span></span>
                        <span class="rightinfo"><input class="input stext laydate-icon needDate" readonly="readonly"
                                                       id="gqjlwcsj" name="gqjlwcsj" type="text"
                                                       value="<s:property value='e.gqjlwcsj.substring(0,10)'/>"/></span>
                    </div>

                    <div class="inputtest inputtestl">
                        <span class="leftname leftnamel lspan" style="width:260px;">净资产（离股权激励完成时间最近）：<span style="color:#f00">*</span></span>
                        <span class="rightinfo rspan" style="width:80px;" ><input  style="background-position:66px center;width:80px;" isNotNull="净资产" isBankZint="isBankZint" onchange="this.value=formatNum(this.value)" isLength="[0,18]" class="input rinput ibg" id="jzc" name="jzc" type="text"
                                                       value="<s:property value='e.jzc'/>"/></span>
                    </div>

                    <div class="inputtest inputtestl">
                        <span class="leftname leftnamel">总股本数(股)：<span style="color:red">*</span></span>
                        <span class="rightinfo"><input isNotNull="总股本数" isZint="总股本数" isLength="[0,18]" class="input" id="cgptcg" name="cgptcg" type="text"
                                                       value="<s:property value='e.cgptcg'/>"/></span>
                    </div>

                    <div class="inputtest inputtestl">
                        <span class="leftname leftnamel">直接持股数(股)：<span style="color:red">*</span></span>
                        <span class="rightinfo"><input isNotNull="直接持股数" isZint="直接持股数" isLength="[0,18]" class="input" id="cgptzgbs" name="cgptzgbs" type="text"
                                                       value="<s:property value='e.cgptzgbs'/>"/></span>
                    </div>

                    <div class="inputtest inputtestl">
                        <span class="leftname leftnamel">申报金额：<span style="color:red">*</span></span>
                        <span class="rightinfo"><input  isNotNull="申报金额" isBankZint="申报金额" onchange="this.value=formatNum(this.value)" isLength="[0,18]" class="input ibg" id="sbje" name="sbje" type="text"
                                                       value="<s:property value='e.sbje'/>"/></span>
                    </div>
                    ...
                    <p class="jiange">持股平台明细</p>

                    <div id="dkk">
                        <c:if test="${fn:length(e.declareYGGQJLCoreList) > 0}">
                            <c:forEach items="${e.declareYGGQJLCoreList}" var="datas" varStatus="status">
                            	<input type="hidden" name="declareYGGQJLCoreList[${status.index}].id" value="${datas.id}">
                                <div class="xkk" lang="firsttr">
                                    
                                    <%--<input type="hidden" lang="关联的TB_SB_YGGQJL表ID" name="declareYGGQJLCoreList[${status.index}].rele_id"--%>
                                           <%--value="${datas.rele_id}"/>--%>

                                    <div class="inputtest inputtestl">
                                        <span class="leftname leftnamel lspan">持股平台持有公司股本数(股)：<span
                                                style="color:red">*</span></span>
                                <span class="rightinfo rspan"><input isNotNull="持股平台持有公司股本数(股)" isZint="持股平台持有公司股本数(股)" isLength="[0,18]" class="input inumber rinput gsgb" id="declareYGGQJLCoreList[${status.index}].cgptcg" name="declareYGGQJLCoreList[${status.index}].cgptcg"
                                                               type="text" value="${datas.cgptcg}"/></span>
                                    </div>

                                    <div class="inputtest inputtestl">
                                        <span class="leftname leftnamel">平台总出资额(元)：<span style="color:red">*</span></span>
                                <span class="rightinfo"><input isNotNull="持股平台总出资额(元)" isZint="持股平台总出资额(元)" isLength="[0,18]" class="input inumber ptgb" id="declareYGGQJLCoreList[${status.index}].cgptzgbs" name="declareYGGQJLCoreList[${status.index}].cgptzgbs"
                                                               type="text"
                                                               value="${datas.cgptzgbs}"/></span>
                                    </div>

                                    <div class="inputtest inputtestl">
                                        <span class="leftname leftnamel lspan">平台中普通员工实缴出资额(元)：<span style="color:red">*</span></span>
                                <span class="rightinfo rspan"><input isNotNull="持股平台中普通员工实缴出资额(元)" isZint="持股平台中普通员工实缴出资额(元)" isLength="[0,18]" class="input inumber rinput yggb" id="declareYGGQJLCoreList[${status.index}].cgptptgbs" name="declareYGGQJLCoreList[${status.index}].cgptptgbs"
                                                               type="text"
                                                               value="${datas.cgptptgbs}"/></span>
                                    </div>
                                    
                                    <div class="inputtest inputtestl">
                                        <span class="leftname leftnamel">持股平台名称：<span style="color:red">*</span></span>
                                <span class="rightinfo rspan"><input isNotNull="持股平台名称" isLength="[1,64]" class="input inumber ptgb" id="declareYGGQJLCoreList[${status.index}].cgptname" name="declareYGGQJLCoreList[${status.index}].cgptname"
                                                               type="text"
                                                               value="${datas.cgptname}"/></span>
                                    </div>

                                    <div style="text-align:right;overflow: hidden;margin-bottom: 10px;clear: both;">
                                        <input class="adddel" type="button" id="copy" value="增加">&nbsp;
                                        <input class="addde2" type="button" id="gbdel" value="删除" onclick="deleteCore(this, ${datas.id})"></div>
                                </div>
                            </c:forEach>
                        </c:if>
                        <c:if test="${fn:length(e.declareYGGQJLCoreList) <= 0}">
                            <div class="xkk" lang="firsttr">
                                ...
                                <div class="inputtest inputtestl">
                                    <span class="leftname leftnamel lspan">持股平台持有公司股本数(股)：<span style="color:red">*</span></span>
                                <span class="rightinfo rspan"><input isNotNull="持股平台持有公司股本数(股)" isZint="持股平台持有公司股本数(股)" isLength="[0,18]"  class="input inumber rinput gsgb" id="declareYGGQJLCoreList[0].cgptcg" name="declareYGGQJLCoreList[0].cgptcg"
                                                               type="text" value=""/></span>
                                </div>

                                <div class="inputtest inputtestl">
                                    <span class="leftname leftnamel">平台总出资额(元)：<span style="color:red">*</span></span>
                                <span class="rightinfo"><input isNotNull="持股平台总出资额(元)" isZint="持股平台总出资额(元)" isLength="[0,18]" class="input inumber ptgb" id="declareYGGQJLCoreList[0].cgptzgbs" name="declareYGGQJLCoreList[0].cgptzgbs"
                                                               type="text"
                                                               value=""/></span>
                                </div>
                                ...
                                <div class="inputtest inputtestl">
                                    <span class="leftname leftnamel lspan">平台中普通员工实缴出资额(元)：<span style="color:red">*</span></span>
                                <span class="rightinfo rspan"><input isNotNull="持股平台中普通员工实缴出资额(元)" isZint="持股平台中普通员工实缴出资额(元)" isLength="[0,18]"  class="input inumber rinput yggb" id="declareYGGQJLCoreList[0].cgptptgbs" name="declareYGGQJLCoreList[0].cgptptgbs"
                                                               type="text"
                                                               value=""/></span>
                                </div>
                                ...
                                <div class="inputtest inputtestl">
                                    <span class="leftname leftnamel">持股平台名称：<span style="color:red">*</span></span>
                                <span class="rightinfo rspan"><input isNotNull="持股平台名称" isLength="[1,64]"  class="input inumber ptgb" id="declareYGGQJLCoreList[0].cgptname" name="declareYGGQJLCoreList[0].cgptname"
                                                               type="text"
                                                               value=""/></span>
                                </div>

                                <div style="text-align:right;overflow: hidden;margin-bottom: 10px;clear: both;"><input
                                        class="adddel" type="button"  value="增加">&nbsp;<input class="addde2"
                                                                                                       type="button"
...
<script type="text/javascript">
    ...
    function checkfilehidden() {
        ...
        picimgupspan.each(function (i, v) {
            ...
            if (!(uid == "aaaaaaaaaaaaaaaaaaaaaaaaatplx368")) {
    ...
    function deleteCore(obj, id) {
    	...
    	$("#delSure .delsure").click(function(){
			...
			if (id != 0 && xkks.length>1) {
					//alert(objid+"&&");
		            $.post('/declare_yggqjl/deleteCore.html',{ id : objid }, function (data) {
