    ...
    $(window).load(function () {
        ...
        if (backnotice == 'success') {
            if(backnoticebzf=='1'){
	            ggts2("数据保存成功！");
	            setTimeout(function(){
	    			window.location="/declare_rzzn/selectAllList.html?type=rzznbt&init=y";
	   			},1500);
		    }
		    if(backnoticebzf=='2'){
		    	ggts2("数据提交成功！");
    ...
    <div class="list fr">
        <div class="rightbox">
            ...
            <div id="zaiquanxinxi-update" class="vipinfo">
                ...
                <form id="form1" action="/declare_rzzn/saveData.html" method="post">
					...
					<div class="inputtest inputtestl">
                        ...
                        <span class="rightinfo"><input isNotNull="企业名称" readonly isLength="[0,64]" class="input" id="qiyemingcheng" name="qiyemingcheng" type="text"
                    ...
                    <p class="jiange">融资租赁明细</p>

                    <div id="dkk">
                        <c:if test="${fn:length(e.declareRZZNCoreList) > 0}">
                            <c:forEach items="${e.declareRZZNCoreList}" var="datas" varStatus="status">
                            	<input type="hidden" name="declareRZZNCoreList[${status.index}].id" value="${datas.id}">
                                <div class="xkk" lang="firsttr">
                                    <%--<input type="hidden" lang="关联的tb_sb_rzzn表ID" name="declareRZZNCoreList[${status.index}].rele_id"--%>
                                           <%--value="${datas.rele_id}"/>--%>



									<div class="inputtest inputtestl">
                                        <span class="leftname leftnamel">租赁机构名称：<span style="color:red">*</span></span>
                                		<span class="rightinfo"><input isNotNull="租赁机构名称" isLength="[0,64]" class="input inumber" id="declareRZZNCoreList[${status.index}].chuzujigou" name="declareRZZNCoreList[${status.index}].chuzujigou"
                                                               type="text"
                                                               value="${datas.chuzujigou}"/></span>
                                    </div>
                                    
                                    <div class="inputtest inputtestl">
                                        <span class="leftname leftnamel">三证合一代码：<span style="color:red">*</span></span>
                                		<span class="rightinfo"><input isNotNull="三证合一代码" isZuzhi="三证合一代码"  isLength="[0,32]" class="input inumber" id="declareRZZNCoreList[${status.index}].chuzujigouzzjgdm" name="declareRZZNCoreList[${status.index}].chuzujigouzzjgdm"
                                                               type="text"
                                                               value="${datas.chuzujigouzzjgdm}"/></span>
                                    </div>
                                    
                                    <div class="inputtest inputtestl">
                                        <span class="leftname leftnamel">租赁标的物：<span style="color:red">*</span></span>
                                		<span class="rightinfo"><input isNotNull="租赁标的物" isLength="[0,16]" class="input inumber" id="declareRZZNCoreList[${status.index}].zhulingwupin" name="declareRZZNCoreList[${status.index}].zhulingwupin"
                                                               type="text"
                                                               value="${datas.zhulingwupin}"/></span>
                                    </div>

								
                                    
                                    <div class="inputtest inputtestl">
                                        <span class="leftname leftnamel">起租时间：<span style="color:red">*</span></span>
                                		<span class="rightinfo"><input class="input stext laydate-icon needDate" readonly="readonly" id="declareRZZNCoreList[${status.index}].qichushijian" name="declareRZZNCoreList[${status.index}].qichushijian"
                                                               type="text"
                                                               value="<fmt:parseDate value="${datas.qichushijian}" var="date1" pattern="yyyy-MM-dd"/><fmt:formatDate value="${date1}" pattern="yyyy-MM-dd"/>"/></span>
                                    </div>
                                    
                                    <div class="inputtest inputtestl">
                                        <span class="leftname leftnamel">结束时间：<span style="color:red">*</span></span>
                                		<span class="rightinfo"><input class="input stext laydate-icon needDate" readonly="readonly" id="declareRZZNCoreList[${status.index}].jieshushijian" name="declareRZZNCoreList[${status.index}].jieshushijian"
                                                               type="text"
                                                               value="<fmt:parseDate value="${datas.jieshushijian}" var="date1" pattern="yyyy-MM-dd"/><fmt:formatDate value="${date1}" pattern="yyyy-MM-dd"/>"/></span>
                                    </div>
                                    
                                    <div class="inputtest inputtestl">
                                        <span class="leftname leftnamel">补贴开开始时间：<span style="color:red">*</span></span>
                                		<span class="rightinfo">
                                		
                                		<%-- <input  class="input stext laydate-icon needDate" id="declareRZZNCoreList[${status.index}].butiekaishiriqi" name="declareRZZNCoreList[${status.index}].butiekaishiriqi"
                                                               type="text"
                                                           value="<fmt:parseDate value="${datas.butiekaishiriqi}" var="date2" pattern="yyyy-MM-dd"/><fmt:formatDate value="${date2}" pattern="yyyy-MM-dd"/>"/> --%>
												<select isnotnull="补贴开开始时间" class="fullval select" id="declareRZZNCoreList[${status.index}].butiekaishiriqi" name="declareRZZNCoreList[${status.index}].butiekaishiriqi"
													val="<fmt:parseDate value="${datas.butiekaishiriqi}" var="date2" pattern="yyyy-MM-dd"/><fmt:formatDate value="${date2}" pattern="yyyy-MM-dd"/>"
													>
													<!-- <option></option> -->
                                    ...
                                    <div class="inputtest inputtestl zhidu">
                                        <span class="leftname leftnamel">补贴结束时间：<span style="color:red">*</span></span>
                                		<span class="rightinfo"><input class="jingzhishijian" readonly="readonly" id="declareRZZNCoreList[${status.index}].butijieshuriqi" name="declareRZZNCoreList[${status.index}].butijieshuriqi"
                                                               type="text"
                                                               value="2018-09-30"/>  </span> 
            
                                    </div>

                                    <div class="inputtest inputtestl">
                                        <span class="leftname leftnamel">首付金：<span style="color:red">*</span></span>
                                    <span class="rightinfo"><input isNotNull="首付金" isBankZint="isBankZint" onchange="this.value=formatNum(this.value)" isLength="[0,18]" class="input ignore inumber ibg" id="declareRZZNCoreList[${status.index}].shoufujin" name="declareRZZNCoreList[${status.index}].shoufujin"
                                                                   type="text" onblur="plus(this, 1)"
                                                                   value="${datas.shoufujin}"/></span>
                                    </div>

                                    <div class="inputtest inputtestl">
                                        <span class="leftname leftnamel">租赁本金：<span style="color:red">*</span></span>
                                    <span class="rightinfo"><input isNotNull="租赁本金" isBankZint="isBankZint" onchange="this.value=formatNum(this.value)" isLength="[0,18]" class="input ignore inumber ibg" id="declareRZZNCoreList[${status.index}].zulinchengben" name="declareRZZNCoreList[${status.index}].zulinchengben"
                                                                   type="text" onblur="plus(this)"
                                                                   value="${datas.zulinchengben}"/></span>
                                    </div>

                                    <div class="inputtest inputtestl zhidu">
                                        <span class="leftname leftnamel">融资额：<span style="color:red">*</span></span>
                                    <span class="rightinfo"><input readonly="readonly" isNotNull="融资额" isBankZint="isBankZint" onchange="this.value=formatNum(this.value)" isLength="[0,18]" class="input ignore inumber ibg" id="declareRZZNCoreList[${status.index}].rongzie" name="declareRZZNCoreList[${status.index}].rongzie"
                                                                   type="text"
                                                                   value="${datas.rongzie}"/></span>
                                    </div>
                                    
                                   	<div class="inputtest inputtestl">
                                        <span class="leftname leftnamel">每期付款金额：<span style="color:red">*</span></span>
                                		<span class="rightinfo"><input isNotNull="每期付款金额" isBankZint="isBankZint" onchange="this.value=formatNum(this.value)" isLength="[0,18]" class="input inumber ibg" id="declareRZZNCoreList[${status.index}].zhulingfeiyong" name="declareRZZNCoreList[${status.index}].zhulingfeiyong"
                                                               type="text"
                                                               value="${datas.zhulingfeiyong}"/></span>
                                    </div>
                                    
                            		     <p style="color:red; clear:both;padding-left:50px">  说明： 融资额=租赁本金-首付金</p>

                                    
                                    <input type="hidden" lang="补贴天数" name="butietianshu" >
                        ...
                        <c:if test="${fn:length(e.declareRZZNCoreList) <= 0}">
                            <div class="xkk" lang="firsttr">
									...
									<div class="inputtest inputtestl">
                                        <span class="leftname leftnamel">租赁机构名称：<span style="color:red">*</span></span>
                                		<span class="rightinfo"><input isNotNull="租赁机构名称" isLength="[0,64]" class="input inumber" id="declareRZZNCoreList[0].chuzujigou" name="declareRZZNCoreList[0].chuzujigou"
                                                               type="text"
                                                               value=""/></span>
                                    </div>
                                    ...
                                    <div class="inputtest inputtestl">
                                        <span class="leftname leftnamel">三证合一代码：<span style="color:red">*</span></span>
                                		<span class="rightinfo"><input isNotNull="三证合一代码"  isZuzhi="三证合一代码"  isLength="[0,32]" class="input inumber" id="declareRZZNCoreList[0].chuzujigouzzjgdm" name="declareRZZNCoreList[0].chuzujigouzzjgdm"
                                                               type="text"
                                                               value=""/></span>
                                    </div>
                                    ...
                                    <div class="inputtest inputtestl">
                                        <span class="leftname leftnamel">租赁标的物：<span style="color:red">*</span></span>
                                		<span class="rightinfo"><input isNotNull="租赁标的物" isLength="[0,16]" class="input inumber" id="declareRZZNCoreList[0].zhulingwupin" name="declareRZZNCoreList[0].zhulingwupin"
                                                               type="text"
                                                               value=""/></span>
                                    </div>
                                    ...
                                    <div class="inputtest inputtestl">
                                        <span class="leftname leftnamel">起租时间：<span style="color:red">*</span></span>
                                		<span class="rightinfo"><input class="input stext laydate-icon needDate" readonly="readonly" id="declareRZZNCoreList[0].qichushijian" name="declareRZZNCoreList[0].qichushijian"
                                                               type="text"
                                                               value=""/></span>
                                    </div>
                                    ...
                                    <div class="inputtest inputtestl">
                                        <span class="leftname leftnamel">结束时间：<span style="color:red">*</span></span>
                                		<span class="rightinfo"><input class="input stext laydate-icon needDate" readonly="readonly" id="declareRZZNCoreList[0].jieshushijian" name="declareRZZNCoreList[0].jieshushijian"
                                                               type="text"
                                                               value=""/></span>
                                    </div>
                                    
                                    <div class="inputtest inputtestl">
                                        <span class="leftname leftnamel">补贴开始时间：<span style="color:red">*</span></span>
                                		<span class="rightinfo">
                                			<!-- <input class="input stext laydate-icon needDate" id="declareRZZNCoreList[0].butiekaishiriqi" name="declareRZZNCoreList[0].butiekaishiriqi"
                                                               type="text" value="2017-04-01"/> -->
                                                               
                                              <select isnotnull="补贴开开始时间" class="select" id="declareRZZNCoreList[0].butiekaishiriqi" name="declareRZZNCoreList[0].butiekaishiriqi">
													<!-- <option></option> -->
													<option value="2017-10-01">2017-10-01</option>
													<!-- <option value="2018-04-01">2018-04-01</option> -->
												</select>
                                    ...
                                    <div class="inputtest inputtestl zhidu">
                                        <span class="leftname leftnamel">补贴结束时间：<span style="color:red">*</span></span>
                                		<span class="rightinfo"><input class="jingzhishijian"  readonly="readonly" id="declareRZZNCoreList[0].butijieshuriqi" name="declareRZZNCoreList[0].butijieshuriqi"
                                                               type="text"
                                                               value="2018-09-30"/></span>
                                    </div>
									...
									<div class="inputtest inputtestl">
                                        <span class="leftname leftnamel">首付金：<span style="color:red">*</span></span>
                                <span class="rightinfo"><input isNotNull="首付金" isBankZint="isBankZint" onchange="this.value=formatNum(this.value)" isLength="[0,18]"
                                                               class="input ignore inumber ibg"
                                                               id="declareRZZNCoreList[0].shoufujin"
                                                               name="declareRZZNCoreList[0].shoufujin"
                                                               type="text" onblur="plus(this, 1)"
                                                               value="" /></span>
                                    </div>
                                    ...
                                    <div class="inputtest inputtestl">
                                        <span class="leftname leftnamel">租赁本金：<span style="color:red">*</span></span>
                                <span class="rightinfo"><input isNotNull="租赁本金" isBankZint="isBankZint" onchange="this.value=formatNum(this.value)" isLength="[0,18]"
                                                               class="input ignore inumber ibg"
                                                               id="declareRZZNCoreList[0].zulinchengben"
                                                               name="declareRZZNCoreList[0].zulinchengben"
                                                               type="text" onblur="plus(this)"
                                                               value="" /></span>
                                    </div>
                                    ...
                                    <div class="inputtest inputtestl  zhidu">
                                        <span class="leftname leftnamel">融资额：<span style="color:red">*</span></span>
                                <span class="rightinfo"><input readonly="readonly"  isNotNull="融资额" isBankZint="isBankZint" onchange="this.value=formatNum(this.value)" isLength="[0,18]"
                                                               class="input ignore inumber ibg"
                                                               id="declareRZZNCoreList[0].rongzie"
                                                               name="declareRZZNCoreList[0].rongzie"
                                                               type="text" readonly
                                                               value="" /></span>
                                    </div>
                               		...
                               		<div class="inputtest inputtestl">
                                        <span class="leftname leftnamel">每期付款金额：<span style="color:red">*</span></span>
                                		<span class="rightinfo"><input isNotNull="每期付款金额" isBankZint="isBankZint" onchange="this.value=formatNum(this.value)" isLength="[0,18]" class="input inumber ibg" id="declareRZZNCoreList[0].zhulingfeiyong" name="declareRZZNCoreList[0].zhulingfeiyong"
                                                               type="text"
                                                               value=""/></span>
                                    </div>
...
//		xkks.eq(0).clone(true).insertAfter($(this).parent().parent()).attr("lang","");//拷贝大框框下的第一个小框框的内容添加到大框框孩子的最后
            newElement.find('input[type=text]').each(function() {
            	...
            	if($(this).attr("class")!="jingzhishijian"){
	                $(this).val('');
            	}
...
//		var lastxkk_inputs = $(this).parent().parent().next().children().find("input");//找到大框框下的最后一个元素里面的所有input
		...
		lastxkk_inputs_buttons.eq(1).attr('onclick',"deleteCore(this, 0)");
            ...
            lastxkk_inputs.each(function () {
                ...
                if ($(this).attr("value") == "增加" || $(this).attr("value") == "删除"||$(this).attr("class")=="jingzhishijian") {
    ...
    function deleteCore(obj, id) {
    	...
    	$("#delSure .delsure").click(function(){
			...
			if (id != 0 && xkks.length>1) {
					//alert(objid+"&&");
		            $.post('/declare_rzzn/deleteCore.html',{ id : objid }, function (data) {
    ...
    function plus(obj, type) {
        var obj = $(obj);
        var shoufujin;
        var zulinchengben;

        if (type == 1) {//首付金失去焦点
            zulinchengben = obj.parent().parent().next().find('input').val().replace(/,/ig, '');
            shoufujin = obj.val().replace(/,/ig, '');
            var rongzie = zulinchengben - shoufujin;
            obj.parent().parent().next().next().find('input').val(formatNum(String(rongzie)));
        } else {
            zulinchengben = obj.val().replace(/,/ig, '');
            shoufujin = obj.parent().parent().prev().find('input').val().replace(/,/ig, '');
            var rongzie = zulinchengben - shoufujin;
            obj.parent().parent().next().find('input').val(formatNum(String(rongzie)));
        }
    }
    $(".fullval").each(function(){
