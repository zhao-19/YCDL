    ...
    $(window).load(function () {
        ...
        if (backnotice == 'success') {
            if(backnoticebzf=='1'){
	            ggts2("数据保存成功！");
	            setTimeout(function(){
	    			window.location="/declare_zwrz/selectAllList.html?type=zzgbbt&init=y";
	   			},1500);
		    }
		    if(backnoticebzf=='2'){
		    	ggts2("数据提交成功！");
    ...
    <div class="list fr">
        <div class="rightbox">
            ...
            <div id="zaiquanxinxi-update" class="vipinfo">
                ...
                <form id="form1" action="/declare_zwrz/saveData.html" method="post">
					...
					<div class="inputtest inputtestl" style="width: 100%;">
                        ...
                        <span class="rightinfo" style="width:620px;"><input readyonly="readonly" style="width:595px;" isNotNull="申报窗口期"
                    ...
                    <div class="inputtest inputtestl">
                        <span class="leftname leftnamel">债务融资类别：<span style="color:red">*</span></span>
                        <span class="rightinfo"><input isNotNull="债务融资类别" isLength="[0,64]" class="input" name="zwrzlb"
                                                       type="text"
                                                       value="<s:property value='e.zwrzlb'/>"/></span>
                    </div>

                    <div class="inputtest inputtestl">
                        <span class="leftname leftnamel">开始日期：<span style="color:red">*</span></span>
                        <span class="rightinfo"><input class="input stext laydate-icon needDate" readonly="readonly" id="kaishiriqi"
                                                       name="kaishiriqi" type="text"
                                                       value="<s:property value='e.kaishiriqi.substring(0,10)'/>"/></span>
                    </div>

                    <div class="inputtest inputtestl">
                        <span class="leftname leftnamel">到期日期：<span style="color:red">*</span></span>
                        <span class="rightinfo"><input class="input stext laydate-icon needDate" readonly="readonly" id="daoqiriqi"
                                                       name="daoqiriqi" type="text" value="<s:property value='e.daoqiriqi.substring(0,10)'/>"/></span>
                    </div>
                    <div class="inputtest inputtestl">
                        <span class="leftname leftnamel">融资金额：<span style="color:red">*</span></span>
                        <span class="rightinfo"><input isNotNull="融资金额" isBankZint="isBankZint" onchange="this.value=formatNum(this.value)" isLength="[0,18]" class="input ibg" id="rongzijine" name="rongzijine" type="text"
                                                       value="<s:property value='e.rongzijine'/>"/></span>
                    </div>
                    <div class="inputtest inputtestl">
                        <span class="leftname leftnamel">融资费用：<span style="color:red">*</span></span>
                        <span class="rightinfo"><input isNotNull="融资费用" isBankZint="isBankZint" onchange="this.value=formatNum(this.value)" isLength="[0,18]" class="input ibg" id="rongzfeiyong" name="rongzfeiyong" type="text"
                                                       value="<s:property value='e.rongzfeiyong'/>"/></span>
                    </div>
                    
                    <div class="inputtest inputtestl">
                        <span class="leftname leftnamel">评级结果：<span style="color:red">*</span></span>
					<span class="rightinfo">
                         <select isNotNull="评级结果" readonly="readonly"  name="e.pingjijieguo" class="select">
                            <option></option>
                            <option value="AAA" <c:if test="${e.pingjijieguo == 'AAA'}">selected</c:if>>AAA</option>
                             <option value="AA+" <c:if test="${e.pingjijieguo == 'AA+'}">selected</c:if>>AA+</option>
                             <option value="AA" <c:if test="${e.pingjijieguo == 'AA'}">selected</c:if>>AA</option>
                             <option value="AA-" <c:if test="${e.pingjijieguo == 'AA-'}">selected</c:if>>AA-</option>
                             <option value="A+" <c:if test="${e.pingjijieguo == 'A+'}">selected</c:if>>A+</option>
                             <option value="A" <c:if test="${e.pingjijieguo == 'A'}">selected</c:if>>A</option>
                             <option value="A-" <c:if test="${e.pingjijieguo == 'A-'}">selected</c:if>>A-</option>
                             <option value="BBB+" <c:if test="${e.pingjijieguo == 'BBB+'}">selected</c:if>>BBB+</option>
                             <option value="BBB" <c:if test="${e.pingjijieguo == 'BBB'}">selected</c:if>>BBB</option>
                             <option value="BBB-" <c:if test="${e.pingjijieguo == 'BBB-'}">selected</c:if>>BBB-</option>
                             <option value="BB+" <c:if test="${e.pingjijieguo == 'BB+'}">selected</c:if>>BB+</option>
                             <option value="BB" <c:if test="${e.pingjijieguo == 'BB'}">selected</c:if>>BB</option>
                             <option value="BB-" <c:if test="${e.pingjijieguo == 'BB-'}">selected</c:if>>BB-</option>
                             <option value="B+" <c:if test="${e.pingjijieguo == 'B+'}">selected</c:if>>B+</option>
                             <option value="B" <c:if test="${e.pingjijieguo == 'B'}">selected</c:if>>B</option>
                             <option value="B-" <c:if test="${e.pingjijieguo == 'B-'}">selected</c:if>>B-</option>
                        </select>
					</span>
...
<script type="text/javascript">
    !function () {
        ...
        laydate({elem: '#kaishiriqi'});//绑定元素
        laydate({elem: '#daoqiriqi'});//绑定元素
    }();
