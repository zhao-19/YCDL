...
<body>
	<s:form action="appDemand!selectList.action" namespace="/"
		...
		<table class="table table-bordered table-condensed">
			...
			<tr>
				...
				<td style="text-align: left;"><s:textfield
						name="apppostdemand.userid" id="inputuser" readonly="false" /></td>
				<td style="text-align: right;">类别</td>
				<td style="text-align: left;"><s:select
						list="%{#request.typeList}" listKey="id" listValue="name"
						name="apppostdemand.gener" label="状态" headerKey="" headerValue="" />
				</td>
			</tr>
			<!--按钮-->
			<tr>
				<td colspan="11">
					<button method="appDemand!selectList.action"
		...
		<table class="table table-bordered table-hover">
			...
			<s:iterator value="pager.list" var="alist">
				<tr>
					...
					<td>&nbsp;${create_time }</td>
					<td>&nbsp;${reviwer_id }</td>
					<td>&nbsp;<s:if test="#alist.ispost=='10001'">是</s:if><s:elseif
							test="#alist.ispost=='10002'">否</s:elseif><s:else>-</s:else></td>
					<td><s:a
							href="appDemand!toEdit.action?e.pkid=%{pkid}&e.gener=%{gener}"
							escapeAmp=""  class="btn btn-mini btn-success">申请详情</s:a>&nbsp;&nbsp; <a
						href="javascript:showReviews('${alist.pkid}','${alist.gener}')"  class="btn btn-mini btn-info">审核记录</a></td>
				</tr>
			</s:iterator>
			<tr>
	...
	<script type="text/javascript">
		...
		function showReviews(pkid,genre) {
			//ajax异步请求查询贷款申请表审核通过数据返回到id为pbczdbaseTablec的table表里面
			//1.查询之后返回到当前
			$.post("appDemand!toReviewLog.action", {
				"pkid" : pkid,"genre":genre
			}, function(data) {
