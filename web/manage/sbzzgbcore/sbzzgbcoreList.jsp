...
<body>
<s:form action="sbzzgbcore!selectList.action" namespace="/" method="post" theme="simple">
	<table class="table table-bordered table-condensed">
		...
		<tr>
			...
			<td style="text-align: right;"></td>
			<td style="text-align: left;" >
				<s:textfield name="e.zzgbcount" id="zzgbcount" readonly="false" />
			</td>
 				
			<td style="text-align: right;"></td>
			<td style="text-align: left;" >
				<s:textfield name="e.stocks" id="stocks" readonly="false" />
			</td>
		...
		<tr>
			<td colspan="11">
				<%if(PrivilegeUtil.check(request.getSession(), "sbzzgbcore!selectList.action")){%>

					<button method="sbzzgbcore!selectList.action" class="btn btn-primary" onclick="selectList(this)">
						<i class="icon-search icon-white"></i> 查询
					</button>
				<%} %>
				<%if(PrivilegeUtil.check(request.getSession(), "sbzzgbcore!insert.action")){%>
					<s:a method="toAdd" cssClass="btn btn-success">
						<i class="icon-plus-sign icon-white"></i> 添加
					</s:a>
				<%} %>
				<%if(PrivilegeUtil.check(request.getSession(), "sbzzgbcore!deletes.action")){%>
		...
		<s:iterator value="pager.list">
			<tr >
				...
				<td>
					...
					<s:a href="sbzzgbcore!toEdit.action?e.id=%{id}">编辑</s:a>
				</td>
			</tr>
		</s:iterator>
