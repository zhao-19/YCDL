...
<s:form action="declare_czbg!selectList.action" namespace="/" method="post" theme="simple">
    <table class="table table-bordered table-condensed">
        ...
        <tr>
            ...
            <td style="text-align: right;">交易对手企业名称</td>
            <td style="text-align: left;">
        ...
        <tr>
            <td colspan="11">
                <%if (PrivilegeUtil.check(request.getSession(), "declare_czbg!selectList.action")) {%>

                <button method="declare_czbg!selectList.action" class="btn btn-primary" onclick="selectList(this)">
                    <i class="icon-search icon-white"></i> 查询
                </button>
                <%} %>
                <%--<%if (PrivilegeUtil.check(request.getSession(), "declare_czbg!insert.action")) {%>--%>
                <%--<s:a method="toAdd" cssClass="btn btn-success">--%>
                    <%--<i class="icon-plus-sign icon-white"></i> 添加--%>
                <%--</s:a>--%>
                <%--<%} %>--%>
                <a href="manage/declare_base!exportExcel.action?type=czbgbt" class="btn btn-success">
                    <i class="icon-white  icon-download-alt"></i>导出
                </a>
                <%if (PrivilegeUtil.check(request.getSession(), "declare_czbg!delete.action")) {%>
    ...
    <table class="table table-bordered table-hover">
        ...
        <s:iterator value="pager.list" var="alist">
            <tr>
                ...
                <td>&nbsp;<s:property value="sqfx"/></td>
                <td>&nbsp;<s:property value="bbgqymc"/></td>
                ...
                <td>
                    ...
                    <s:a href="declare_czbg!toEdit.action?e.id=%{id}" class="btn btn-mini btn-info"><i class="icon-white icon-check"></i>审核</s:a>
                    <a href="javascript:showReviews('${alist.id}','3b1ae2ef7d714f9aa0d23330aaaecc60')" class="btn btn-mini btn-success">查看审核记录</a>
                </td>
            </tr>
        </s:iterator>
		</tbody>
    ...
    function deletes() {
        ...
        msg.confirm("是否要执行此操作？", function () {
            ...
            if (checkLength <= 0) {
                ...
                $.post("/manage/declare_czbg!delete.action", ids.serialize(), function (data) {
