...
<s:form action="declare_pj!selectList.action" namespace="/" method="post" theme="simple">
    <table class="table table-bordered table-condensed">
        ...
        <tr>
            ...
            <td style="text-align: right;">评级机构</td>
            <td style="text-align: left;">
                <s:textfield name="e.pjorg" id="pjorg" readonly="false"/>
            </td>
        ...
        <tr>
            <td colspan="11">
                <%if (PrivilegeUtil.check(request.getSession(), "declare_pj!selectList.action")) {%>

                <button method="declare_pj!selectList.action" class="btn btn-primary" onclick="selectList(this)">
                    <i class="icon-search icon-white"></i> 查询
                </button>

                <%} %>
                <%--<%if (PrivilegeUtil.check(request.getSession(), "declare_pj!insert.action")) {%>--%>
                <%--<s:a method="toAdd" cssClass="btn btn-success">--%>
                    <%--<i class="icon-plus-sign icon-white"></i> 添加--%>
                <%--</s:a>--%>
                <%--<%} %>--%>
                <a href="manage/declare_base!exportExcel.action?type=xypjbt" class="btn btn-success">
                    <i class="icon-white  icon-download-alt"></i> 导出
                </a>
                <%if (PrivilegeUtil.check(request.getSession(), "declare_pj!delete.action")) {%>
    ...
    <table class="table table-bordered table-hover">
        ...
        <s:iterator value="pager.list" var="alist">
            <tr>
               ...
               <%--  <td>&nbsp;<s:property value="zcfx"/></td> --%>
                <td>&nbsp;<s:property value="pjorg"/></td>
                ...
                <td>
                    ...
                    <s:a href="declare_pj!toEdit.action?e.id=%{id}" class="btn btn-mini btn-info"><i class="icon-white icon-check"></i>审核</s:a>
                    <a href="javascript:showReviews('${alist.id}','76e15fc2cd464f0c8c193484e1514636')" class="btn btn-mini btn-success">查看审核记录</a>
                </td>
            </tr>
        </s:iterator>
		</tbody>
    ...
    function deletes() {
        ...
        msg.confirm("是否要执行此操作？", function () {
            ...
            if (checkLength <= 0) {
                ...
                $.post("/manage/declare_pj!delete.action", ids.serialize(), function (data) {
