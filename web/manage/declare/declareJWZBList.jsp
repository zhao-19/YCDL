...
<s:form action="declare_jwzb!selectList.action" namespace="/" method="post" theme="simple">
    <table class="table table-bordered table-condensed">
        ...
        <tr>
            <td colspan="11">
                <%if (PrivilegeUtil.check(request.getSession(), "declare_jwzb!selectList.action")) {%>

                <button method="declare_jwzb!selectList.action" class="btn btn-primary" onclick="selectList(this)">
                    <i class="icon-search icon-white"></i> 查询
                </button>
                <%} %>
                <%--<%if (PrivilegeUtil.check(request.getSession(), "declare_jwzb!insert.action")) {%>--%>
                <%--<s:a method="toAdd" cssClass="btn btn-success">--%>
                    <%--<i class="icon-plus-sign icon-white"></i> 添加--%>
                <%--</s:a>--%>
                <%--<%} %>--%>
                <a href="manage/declare_base!exportExcel.action?type=jwsscgbt" class="btn btn-success">
                    <i class="icon-white  icon-download-alt"></i>导出
                </a>
                <%if (PrivilegeUtil.check(request.getSession(), "declare_jwzb!delete.action")) {%>
        ...
        <s:iterator value="pager.list" var="alist">
            <tr>
                ...
                <td>
                    ...
                    <s:a href="declare_jwzb!toEdit.action?e.id=%{id}" class="btn btn-mini btn-info"><i class="icon-white icon-check"></i>审核</s:a>
                    <a href="javascript:showReviews('${alist.id}','19405a8899fa44e1aeb2b2233d9e05f8')" class="btn btn-mini btn-success">查看审核记录</a>
                </td>
            </tr>
        </s:iterator>
        </tbody>
    ...
    function deletes() {
        ...
        msg.confirm("是否要执行此操作？", function () {
            ...
            if (checkLength <= 0) {
                ...
                $.post("/manage/declare_jwzb!delete.action", ids.serialize(), function (data) {
