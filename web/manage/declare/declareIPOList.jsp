...
<s:form action="declare_ipo!selectList.action" namespace="/" method="post" theme="simple">
    <table class="table table-bordered table-condensed">
        ...
        <tr>
            ...
            <td style="text-align: right;">拟上市交易所</td>
            <td style="text-align: left;">
                <s:textfield name="e.nssjys" id="nssjys" readonly="false"/>
            </td>
        ...
        <tr>
            <td colspan="11">
                <%if (PrivilegeUtil.check(request.getSession(), "declare_ipo!selectList.action")) {%>

                <button method="declare_ipo!selectList.action" class="btn btn-primary" onclick="selectList(this)">
                    <i class="icon-search icon-white"></i> 查询
                </button>
                <%} %>
                <%--<%if (PrivilegeUtil.check(request.getSession(), "declare_ipo!insert.action")) {%>--%>
                <%--<s:a method="toAdd" cssClass="btn btn-success">--%>
                    <%--<i class="icon-plus-sign icon-white"></i> 添加--%>
                <%--</s:a>--%>
                <%--<%} %>--%>
                <a href="manage/declare_base!exportExcel.action?type=ipohjbt" class="btn btn-success">
                    <i class="icon-white  icon-download-alt"></i>导出
                </a>
                <%if (PrivilegeUtil.check(request.getSession(), "declare_ipo!delete.action")) {%>
    ...
    <table class="table table-bordered table-hover">
        ...
        <s:iterator value="pager.list" var="alist">
            <tr>
                ...
                <td>&nbsp;<% TreeNode t = SystemManager.treeNodeMap.get(request.getAttribute("nssjys")); if( t != null){
                    out.print(t.getName());}%></td>
                <td>&nbsp;<s:date name="slsj" format="yyyy-MM-dd"/></td>
                ...
                <td>
                    ...
                    <s:a href="declare_ipo!toEdit.action?e.id=%{id}" class="btn btn-mini btn-info"><i class="icon-white icon-check"></i>审核</s:a>
                    <a href="javascript:showReviews('${alist.id}','c97d10ee282c4e7492c6dfca2d8a81d8')" class="btn btn-mini btn-success">查看审核记录</a>
                </td>
            </tr>
        </s:iterator>
	</tbody>
    ...
    function deletes() {
        ...
        msg.confirm("是否要执行此操作？", function () {
            ...
            if (checkLength <= 0) {
                ...
                $.post("/manage/declare_ipo!delete.action", ids.serialize(), function (data) {
