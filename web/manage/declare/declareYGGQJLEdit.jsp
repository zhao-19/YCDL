...
<s:form action="declare_yggqjl" namespace="/manage" theme="simple" id="form" lang="isBank">
    <table class="table table-bordered">
        ...
        <tr>
            ...
            <th class="td_right">股权激励完成时间</th>
            <td style="text-align: left;">
                <s:textfield name="e.gqjlwcsj" id="gqjlwcsj" readonly="false"
                             data-rule="股权激励完成时间:required;gqjlwcsj;"/>
            </td>
        </tr>

        <tr>
            <th class="td_right">净资产（离股权激励完成时间最近）（万元）</th>
            <td style="text-align: left;">
                <s:textfield name="e.jzc" isBankZint="isBankZint" onchange="this.value=formatNum(this.value)" id="jzc" readonly="false"
                             data-rule="净资产（离股权激励完成时间最近:required;jzc;length[0~9]"/>
            </td>
            <th class="td_right">总股本数</th>
            <td style="text-align: left;">
                <s:textfield name="e.cgptcg" id="cgptcg" readonly="false" data-rule="总股本数:required;cgptcg;length[0~11]"/>
            </td>
        </tr>

        <tr>
            <th class="td_right">直接持股数</th>
            <td style="text-align: left;">
                <s:textfield name="e.cgptzgbs" id="cgptzgbs" readonly="false" />
            </td>
            <th class="td_right">申报金额（万元）</th>
            <td style="text-align: left;">
                <s:textfield name="e.sbje" isBankZint="isBankZint" onchange="this.value=formatNum(this.value)" id="sbje" readonly="false" data-rule="申报金额:required;sbje;length[0~9]"/>
            </td>
        </tr>
	 ...
	 <td colspan="4" style="color:red">
        按员工所得股权对应净资产的5%给予企业最高不超过50万元的补贴
	</td>
	</tr>
    ...
    <table class="table table-condensed">
        ...
        <tr style="background-color: #dff0d8">
            ...
            <th>持股平台持有公司股本数</th>
            <th>持股平台总出资额</th>
            <th>持股平台中普通员工实缴出资额</th>
            <th>操作</th>
        </tr>
        </thead>
        <s:iterator value="e.declareYGGQJLCoreList">
            <tr>
                <td>&nbsp;<s:property value="userid"/></td>
                <td>&nbsp;<s:property value="cgptname"/></td>
                <td>&nbsp;<s:property value="cgptcg"/></td>
                <td>&nbsp;<s:property value="cgptzgbs"/></td>
                <td>&nbsp;<s:property value="cgptptgbs"/></td>
        ...
        <tr>
            <td colspan="4" style="text-align: center;">
                ...
                <s:if test="e.id=='' or e.id==null">
                    <button method="declare_yggqjl!insert.action" class="btn btn-success">
                        <i class="icon-ok icon-white"></i> 新增
                    </button>
                </s:if>
                <s:else>
                    <button method="declare_yggqjl!update.action" class="btn btn-success">
                        <i class="icon-ok icon-white"></i> 保存
                    </button>
                    &nbsp;&nbsp;&nbsp;&nbsp;
                    <span method="declare_yggqjl!update.action" id="backButton"  onclick="statusset('0')" class="btn btn-danger">
                        <i class="icon-ok icon-white"></i> 驳回
                    </span>
                    &nbsp;&nbsp;&nbsp;&nbsp;

                    <span method="declare_yggqjl!update.action" id="successButton" onclick="statusset('1')" class="btn btn-info">
    ...
    <div id="ldksq">
        <div id="ldksq_c">
            ...
            <p>
                ...
                <button  method="declare_yggqjl!update.action" class="btn btn-success" style="margin-left:20px;">
...
//        }
    ...
    function toCodeEdit(id) {
        layer.open({
            ...
            fix: false, //不固定
            maxmin: false,
            content: 'declare_yggqjl_core!toEdit.action?id=' + id
        });
    }
    ...
    $(function () {
        ...
        $("#successButton").click(function () {
            var content = lxr + "您好，"+ qiyemingcheng +"申请的员工股权激励补贴获得成都高新区财政金融局初审通过，请根据操作手册说明从系统导出资料，双面打印并胶装，签字盖章后交到锦城大道539号盈创动力大厦A座五楼且带上财务章（用于收据盖章）。如有疑问请联系申报通知中对应的联系人。";
        ...
        $("#backButton").click(function () {
            ...
            var content = lxr + "您好，很遗憾"+ qiyemingcheng +"申请的员工股权激励补贴未获得成都高新区财政金融局初审通过，原因：xxxxxxxxxxxxxxxxxxxx。如有疑问请联系申报通知中对应的联系人。";
