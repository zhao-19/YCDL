...
<s:form action="declare_btz!selectList.action" namespace="/" method="post" theme="simple">
    <table class="table table-bordered table-condensed">
        ...
        <tr>
            ...
            <td style="text-align: right;">申报类别</td>
            <td style="text-align: left;">
                <s:select list="%{#request.leibie}" listKey="id"
                          listValue="name" name="e.leibie" label="申报类别"
                          headerKey="" headerValue="" />
            </td>

            <td style="text-align: right;">联系人手机</td>
            <td style="text-align: left;">
                <s:textfield name="e.lxrsj" id="lxrsj" readonly="false"/>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            <td colspan="11">
                <%if (PrivilegeUtil.check(request.getSession(), "declare_btz!selectList.action")) {%>

                <button method="declare_btz!selectList.action" class="btn btn-primary" onclick="selectList(this)">
                    <i class="icon-search icon-white"></i> 查询
                </button>
                <%} %>
                <%--<%if (PrivilegeUtil.check(request.getSession(), "declare_btz!insert.action")) {%>--%>
                <%--<s:a method="toAdd" cssClass="btn btn-success">--%>
                    <%--<i class="icon-plus-sign icon-white"></i> 添加--%>
                <%--</s:a>--%>
                <%--<%} %>--%>
                <a href="manage/declare_base!exportExcel.action?type=qybtz" class="btn btn-success">
                    <i class="icon-white  icon-download-alt"></i>导出
                </a>
                <%if (PrivilegeUtil.check(request.getSession(), "declare_btz!delete.action")) {%>
    ...
    <table class="table table-bordered table-hover">
        ...
        <s:iterator value="pager.list" var="alist">
            <tr>
                ...
                <td>&nbsp;<s:if test="leibie == 1">移动互联网</s:if><s:else>科技创新</s:else></td>
                <td>&nbsp;<s:property value="qiyemingcheng"/></td>
                <td>&nbsp;<s:property value="lxr"/></td>
                <td>&nbsp;<s:property value="lxrsj"/></td>
                ...
                <td>
                    ...
                    <s:a href="declare_btz!toEdit.action?e.id=%{id}" class="btn btn-mini btn-info"><i class="icon-white icon-check"></i>审核</s:a>
<a href="javascript:showReviews('${alist.id}','3eff48cae44a41b1bb570b13b631a5b7')" class="btn btn-mini btn-success"></i>查看审核记录</a>
                </td>
            </tr>
        </s:iterator>
		</tbody>
    ...
    function deletes() {
        ...
        msg.confirm("是否要执行此操作？", function () {
            ...
            if (checkLength <= 0) {
                ...
                $.post("/manage/declare_btz!delete.action", ids.serialize(), function (data) {
