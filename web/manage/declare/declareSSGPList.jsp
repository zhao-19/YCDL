...
<s:form action="declare_ssgp!selectList.action" namespace="/" method="post" theme="simple">
    <table class="table table-bordered table-condensed">
        ...
        <tr>
            ...
            <td style="text-align: right;">股票代码</td>
            <td style="text-align: left;">
                <s:textfield name="e.gpdm" id="gpdm" readonly="false"/>
            </td>
        ...
        <tr>
            <td colspan="11">
                <%if (PrivilegeUtil.check(request.getSession(), "declare_ssgp!selectList.action")) {%>

                <button method="declare_ssgp!selectList.action" class="btn btn-primary" onclick="selectList(this)">
                    <i class="icon-search icon-white"></i> 查询
                </button>
                <%} %>
                <%--<%if (PrivilegeUtil.check(request.getSession(), "declare_ssgp!insert.action")) {%>--%>
                <%--<s:a method="toAdd" cssClass="btn btn-success">--%>
                    <%--<i class="icon-plus-sign icon-white"></i> 添加--%>
                <%--</s:a>--%>
                <%--<%} %>--%>
                <a href="manage/declare_base!exportExcel.action?type=sscgbt" class="btn btn-success">
                    <i class="icon-white  icon-download-alt"></i>导出
                </a>
                <%if (PrivilegeUtil.check(request.getSession(), "declare_ssgp!delete.action")) {%>
        ...
        <s:iterator value="pager.list" var="alist">
            <tr>
                ...
                <td>
                    ...
                    <s:a href="declare_ssgp!toEdit.action?e.id=%{id}" class="btn btn-mini btn-info"><i class="icon-white icon-check"></i>审核</s:a>
                    <a href="javascript:showReviews('${alist.id}','f17ec4b7496b41a99a8362432c377a97')" class="btn btn-mini btn-success">查看审核记录</a>
                </td>
            </tr>
        </s:iterator>
		</tbody>
    ...
    function deletes() {
        ...
        msg.confirm("是否要执行此操作？", function () {
            ...
            if (checkLength <= 0) {
                ...
                $.post("/manage/declare_ssgp!delete.action", ids.serialize(), function (data) {
