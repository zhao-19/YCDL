...
<s:form action="declare_zwrz!selectList.action" namespace="/" method="post" theme="simple">
    <table class="table table-bordered table-condensed">
        ...
        <tr>
            <td colspan="11">
                <%if (PrivilegeUtil.check(request.getSession(), "declare_zwrz!selectList.action")) {%>

                <button method="declare_zwrz!selectList.action" class="btn btn-primary" onclick="selectList(this)">
                    <i class="icon-search icon-white"></i> 查询
                </button>
                <%} %>
                <%--<%if (PrivilegeUtil.check(request.getSession(), "declare_zwrz!insert.action")) {%>--%>
                <%--<s:a method="toAdd" cssClass="btn btn-success">--%>
                    <%--<i class="icon-plus-sign icon-white"></i> 添加--%>
                <%--</s:a>--%>
                <%--<%} %>--%>
                <a href="manage/declare_base!exportExcel.action?type=zwrzbt" class="btn btn-success">
                    <i class="icon-white  icon-download-alt"></i>导出
                </a>
                <%if (PrivilegeUtil.check(request.getSession(), "declare_zwrz!delete.action")) {%>
    ...
    <table class="table table-bordered table-hover">
        ...
        <s:iterator value="pager.list" var="alist">
            <tr>
                ...
                <td>&nbsp;<s:property value="zwrzlb"/></td>
                <td>&nbsp;<s:property value="pingjijieguo"/></td>
                ...
                <td>
                    ...
                    <s:a href="declare_zwrz!toEdit.action?e.id=%{id}" class="btn btn-mini btn-info"><i class="icon-white icon-check"></i>审核</s:a>
<a href="javascript:showReviews('${alist.id}','b9f522a9b43f4d2e96e848dfd917921f')" class="btn btn-mini btn-success">查看审核记录</a>
                </td>
            </tr>
        </s:iterator>
		</tbody>
...
function checkUndefined(a){
    ...
    function deletes() {
        ...
        msg.confirm("是否要执行此操作？", function () {
            ...
            if (checkLength <= 0) {
                ...
                $.post("/manage/declare_zwrz!delete.action", ids.serialize(), function (data) {
    ...
    $("#zwrztbody tr").each(function(){
		var text = $.trim($(this).find("td:eq(6)").text());
		if (text.length == 10 && text.replace("-", "").substr(0, 6) < ${zcsbpichi}) {
