	...
	<div style="text-align: right; border: 0;padding: 4px 12px;" class="pageDiv">
		<pg:pager url="${pager.pagerUrl}" items="${pager.total}"
			...
			maxPageItems="${pager.pageSize}" maxIndexPages="10" isOffset="true">
					<pg:param name="querytype" value="${querytype}"></pg:param>
			...
			<pg:pages>
				<c:choose>
						<c:when test="${currentPageNumber eq pageNumber and (currentPageNumber+2 ge pageNumber and currentPageNumber-2 le pageNumber)}">
							<span class="selectPageLink">${pageNumber}</span>
						</c:when>
						<c:when  test="${currentPageNumber ne pageNumber and (currentPageNumber+2 ge pageNumber and currentPageNumber-2 le pageNumber)}">
						  	<a href="${pageUrl}" class="pageLink">${pageNumber}</a>
						<!--	<a href="javaScript:pager(${pageNumber-1}*${pager.pageSize });" class="pageLink">${pageNumber}</a>-->
						</c:when>
				</c:choose>
			</pg:pages>
			<pg:next>
