...
<body>
<s:form action="guquanhuangtai!selectList.action" namespace="/" method="post" theme="simple">
	<table class="table table-bordered table-condensed">
		...
		<tr>
			<td colspan="11">
				<%if(PrivilegeUtil.check(request.getSession(), "guquanhuangtai!selectList.action")){%>

					<button method="guquanhuangtai!selectList.action" class="btn btn-primary" onclick="selectList(this)">
						<i class="icon-search icon-white"></i> 查询
					</button>
				<%} %>
				<%if(PrivilegeUtil.check(request.getSession(), "guquanhuangtai!insert.action")){%>
					<s:a method="toAdd" cssClass="btn btn-success">
						<i class="icon-plus-sign icon-white"></i> 添加
					</s:a>
				<%} %>
				<%if(PrivilegeUtil.check(request.getSession(), "guquanhuangtai!deletes.action")){%>
		...
		<s:iterator value="pager.list">
			<tr >
				...
				<td>
					...
					<s:a href="guquanhuangtai!toEdit.action?e.id=%{id}" class="btn btn-mini btn-info"><i class="icon-white icon-edit"></i>编辑</s:a>
				</td>
			</tr>
		</s:iterator>
