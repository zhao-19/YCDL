...
<body>
<s:form action="productczdgysxssxx!selectList.action" namespace="/" method="post" theme="simple">
	<table class="table table-bordered table-condensed">
		...
		<tr>
			...
			<td style="text-align: right;">类型（供应商/销售商）</td>
			<td style="text-align: left;" >
				<s:textfield name="e.custype" id="custype" readonly="false" />
			</td>
 				
			<td style="text-align: right;">市场集中区域</td>
			<td style="text-align: left;" >
				<s:textfield name="e.shichangquyu" id="shichangquyu" readonly="false" />
			</td>
 				
			<td style="text-align: right;">公司名称</td>
			<td style="text-align: left;" >
				<s:textfield name="e.gongshingmingcheng" id="gongshingmingcheng" readonly="false" />
			</td>
 				
			<td style="text-align: right;">商品名称</td>
			<td style="text-align: left;" >
				<s:textfield name="e.shagnminmingcheng" id="shagnminmingcheng" readonly="false" />
			</td>
 				
			<td style="text-align: right;">金额</td>
			<td style="text-align: left;" >
				<s:textfield name="e.jine" id="jine" readonly="false" />
			</td>
		...
		<tr>
			<td colspan="11">
				<%if(PrivilegeUtil.check(request.getSession(), "productczdgysxssxx!selectList.action")){%>

					<button method="productczdgysxssxx!selectList.action" class="btn btn-primary" onclick="selectList(this)">
						<i class="icon-search icon-white"></i> 查询
					</button>
				<%} %>
				<%if(PrivilegeUtil.check(request.getSession(), "productczdgysxssxx!insert.action")){%>
					<s:a method="toAdd" cssClass="btn btn-success">
						<i class="icon-plus-sign icon-white"></i> 添加
					</s:a>
				<%} %>
				<%if(PrivilegeUtil.check(request.getSession(), "productczdgysxssxx!deletes.action")){%>
		...
		<s:iterator value="pager.list">
			<tr >
				...
				<td>
					...
					<s:a href="productczdgysxssxx!toEdit.action?e.id=%{id}">编辑</s:a>
				</td>
			</tr>
		</s:iterator>
