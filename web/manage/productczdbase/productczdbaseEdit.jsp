    ...
    function refresh(value){
		...
		<div id="tabs-1">
			  ...
			  <tr >
				...
				<s:select list="#{'国家开发银行':'国家开发银行','进出口银行':'进出口银行','盈创动力分配银行':'盈创动力分配银行','成都银行科技支行':'成都银行科技支行','成都银行高新支行':'成都银行高新支行','盈创动力分配银行':'盈创动力分配银行','成都银行营业部':'成都银行营业部','成都银行华兴支行':'成都银行华兴支行','成都银行高新支行':'成都银行高新支行','成都银行琴台支行':'成都银行琴台支行','成都银行金河支行':'成都银行金河支行','成都银行长顺支行':'成都银行长顺支行','成都银行锦江支行':'成都银行锦江支行','成都银行成华支行':'成都银行成华支行','成都银行武侯支行':'成都银行武侯支行','成都银行德盛支行':'成都银行德盛支行','成都银行青羊支行':'成都银行青羊支行','成都银行高升桥支行':'成都银行高升桥支行','成都银行金牛支行':'成都银行金牛支行','成都银行沙湾支行':'成都银行沙湾支行','成都银行体育场路支行':'成都银行体育场路支行','成都银行科技支行':'成都银行科技支行','成都银行双流支行':'成都银行双流支行','成都银行龙泉驿支行':'成都银行龙泉驿支行','成都银行彭州支行':'成都银行彭州支行','成都银行邛崃支行':'成都银行邛崃支行','成都银行新津支行':'成都银行新津支行','成都银行郫县支行':'成都银行郫县支行','成都银行都江堰支行':'成都银行都江堰支行','成都银行温江支行':'成都银行温江支行','成都银行崇州支行':'成都银行崇州支行','成都银行新都支行':'成都银行新都支行','成都银行大邑支行':'成都银行大邑支行','成都银行青白江支行':'成都银行青白江支行','成都银行金堂支行':'成都银行金堂支行','成都银行蒲江支行':'成都银行蒲江支行','工行高新支行':'工行高新支行','邮储银行成都市高新区支行':'邮储银行成都市高新区支行','邮储银行成都市太升南路支行':'邮储银行成都市太升南路支行','邮储银行成都市高新支行':'邮储银行成都市高新支行','邮储银行省分行直属支行':'邮储银行省分行直属支行','邮储银行成都市沙湾支行':'邮储银行成都市沙湾支行','邮储银行成都市双楠支行':'邮储银行成都市双楠支行','建设银行成都第五支行':'建设银行成都第五支行','工行双流支行':'工行双流支行'}" id="daikuanyinhang" name="e.daikuanyinhang"  cssClass="input-medium" 
		 ...
		 <!-- 其他信息 -->
			<div id="tabs-7">
 			...
 			<tr>
				...
				<td style="text-align: left;" colspan="3"> 
				<s:textarea rows="4" cols="80" name="qtxx.chanpinjingzhengli" id="chanpinjingzhengli" readonly="false" data-rule="产品竞争力:required;chanpinjingzhengli;length[1~8000]"/>
				</td>
				</tr>
				...
				<td style="text-align: left;" colspan="3">
				<s:textarea rows="4" cols="80" name="qtxx.gaoguantuandui" id="gaoguantuandui" readonly="false" data-rule="高管团队:required;gaoguantuandui;length[1~8000]"/>
				</td>
				</tr>
...
if ('${msg}' != "") {
	layer.alert('${msg}', {icon: 7, title : '提示', offset: ['20%', '35%']});
}
...
<div id="ldksq">
    <div id="ldksq_c">
        ...
        <p id="tckselect" style="display:none;margin-top: 20px;"><span>贷款产品：</span><span><s:select list="#{'成长贷':'成长贷','壮大贷':'壮大贷','新创贷':'新创贷','园保贷':'园保贷','开行统借统还贷款':'开行统借统还贷款','口行统借统还贷款':'口行统借统还贷款'}" id="daikuanpingz" name="e.daikuanpingz"  cssClass="input-medium" 
										listKey="key" listValue="value"  headerKey="" headerValue=""  data-rule="贷款品种:required;daikuanpingz"/></span></p>
        <p id="tcktel"><span>电话号码：</span><input class="title" type="text"   value="<s:property value="e.dianhua" />" name="e.dianhua" data-rule="短信标题:required;m_title;length[2~22]"></p>
        <p id="tckremark"><span class="msgcontenttype">短信内容：</span><textarea class="duanxin" rows="4" cols="80" name="e.content" id="msgcontent"   data-rule="短信标题:required;m_remark;length[2~512]">   </textarea></p>
        <p>
