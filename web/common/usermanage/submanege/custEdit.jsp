...
<s:form action="/usermanage/saveCust.html" id="form" name="form" theme="simple" method="post">
    ...
    <input type="hidden" value="<s:property value="cust.id"/>" id="productID"/>
    <input type="hidden" value="<s:property value="cust.catalogID"/>" id="catalogID"/>


    <div style="text-align: center; margin-bottom: 10px;">
        <div id="updateMsg"><font color='red'><s:property value="updateMsg"/></font></div>
            用户ID：<span class="badge badge-success"><s:property value="cust.id"/></span>
    ...
    <div id="tabs">
        ...
        <ul>
            ...
            <s:if test='#request.cust.usertype=="aaaaaaaaaaaaaaaaaaaaaaaaaaaaa001"'>
                <li><a href="#tabs-2">企业信息</a></li>
                <li><a href="#tabs-6">财务信息</a></li>
            </s:if>
           
            <s:if test='#request.cust.usertype=="aaaaaaaaaaaaaaaaaaaaaaaaaaaaa003"'>
                <li><a href="#tabs-4">机构信息</a></li>
            </s:if>
            <s:if test='#request.cust.usertype=="aaaaaaaaaaaaaaaaaaaaaaaaaaaaa008"'>
        ...
        <div id="tabs-1">
            <table class="table table-condensed table-bordered">
                <tr style="display:none;">
                    <th colspan="2">id</th>
                    <td colspan="2"><s:hidden name="cust.id"/></td>
                </tr>
                <tr>
                    <th class="td_right">用户名</th>
                    <td style="text-align: left;">
                        <s:textfield name="cust.userid" id="userid" readonly="true"/>
                ...
                <tr>
                    ...
                    <td style="text-align: left;" colspan="3">
                        <s:textfield name="cust.errorcount" id="errorcount" readonly="false"/>
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td style="text-align: left;">
                        ...
                        <s:textfield name="cust.email" id="email" readonly="false" size="32" maxlength="32"/>
                    </td>

                    <th class="td_right">是否注销</th>
                    <td style="text-align: left;">
                        <s:select list="%{#application.dic.whether.childs}" listKey="id" id="custdisable"
                                  listValue="name" name="cust.disable" label="状态" headerKey="" headerValue=""
                        />
                   ...
                   <%--  <th class="td_right">是否审核</th>
                    <td style="text-align: left;">
                        <s:select list="%{#request.status2list}" listKey="id"
                                  listValue="name" name="cust.status2" label="状态"/>
                    </td> --%>
                </tr>
                <tr>
                    <th class="td_right">禁言时间</th>
                    <td style="text-align: left;">
                        <input id="forbidend" class="Wdate search-query input-small" type="text" name="cust.forbidend"
						value="${fn:substring(cust.forbidend, 0, 10)}" onFocus="WdatePicker()" />
                ...
                <tr>
                    ...
                    <td style="text-align: left;">
                        <s:property value="cust.adddate"/>

                    </td>

                    <th class="td_right">上次登录时间</th>
                    <td style="text-align: left;">
                        <s:property value="cust.lastlogintime"/>
                ...
                <tr>
                    ...
                    <td style="text-align: left;" colspan="3">
                        <s:property value="cust.custmanagesource"/>
                    </td>
                </tr>
 ...
 <!-- 个人 -->
       <%--  <s:if test='#request.cust.usertype=="aaaaaaaaaaaaaaaaaaaaaaaaaaaaa002"'> --%>
		...
		<!-- 财务信息 -->
			<div id="tabs-6">
				<div style="padding: 4px 5px; color: #999;">*财务情况（万元）</div>
...
<script type="text/javascript">
	...
	if ('${cust.usertype}' == 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaa008') {
		$.ajax({
			type: 'post',
...
<script>
    ...
    KindEditor.ready(function (K) {
        ...
        editor1 = K.create('textarea[name="cust.productargHTML"]', {
            allowFileManager: true
        });
        editor2 = K.create('textarea[name="cust.packagelistHTML"]', {
            allowFileManager: true
        });
        editor3 = K.create('textarea[name="cust.serviceHTML"]', {
            allowFileManager: true
        });
        editor4 = K.create('textarea[name="cust.productHTML"]', {
            allowFileManager: true
        });
        K('input[name=getHtml]').click(function (e) {
            alert(editor1.html());
...
<script>
    ...
    function resetpassword() {
	    if (confirm("确认重置密码？")) {
	    	$.ajax({
				type: 'post',
				url: "/usermanage/resetpassword.html",
				data: {"id": '${cust.id}'},
				dataType:"json",
				success: function(data){
					if (data.msg = "succ") {
    ...
    function modifycellphone() {
	    if (confirm("确认修改手机号？")) {
	    	...
	    	$.ajax({
				type: 'post',
				url: "/usermanage/modifycellphone.html",
				data: {"id": '${cust.id}',"cellphone": cellphone},
				dataType:"json",
				success: function(data){
					if (data.msg = "succ") {
						alert("手机号修改成功!");
    ...
    function modifyusertype() {
	    if (confirm("确认修改用户类型？")) {
	    	...
	    	$.ajax({
				type: 'post',
				url: "/usermanage/modifyusertype.html",
				data: {"id": '${cust.id}',"usertype": usertype, "userid" : '${cust.userid}'},
				dataType:"json",
				success: function(data){
					if (data.msg = "succ") {
						alert("用户类型修改成功!");
						location.href = "/usermanage/toCustEdit.html?id=${cust.id}";
					} else {
						alert(data.msg)
					}
				}
    ...
    function disablecust() {
	    if (confirm("确认修改注销状态？")) {
	    	...
	    	$.ajax({
				type: 'post',
				url: "/usermanage/disablecust.html",
				data: {"id": '${cust.id}',"disable": custdisable},
				dataType:"json",
				success: function(data){
					if (data.msg = "succ") {
						alert("注销状态修改成功!");
    ...
    function forbidendcust() {
	    if (confirm("确认修改禁言时间？")) {
	    	...
	    	$.ajax({
				type: 'post',
				url: "/usermanage/forbidendcust.html",
				data: {"id": '${cust.id}',"forbidend": forbidendcust},
				dataType:"json",
				success: function(data){
					if (data.msg = "succ") {
						alert("禁言时间修改成功!");
    ...
    function custchild() {
    	layer.open({
            ...
            fix: false, //不固定
            maxmin: false,
            content: "/usermanage/custchild.html?id=${cust.id}"
        });
    }
    ...
    function qiyeCheckCreditId(obj){
    	...
    	$.ajax({
			type: 'post',
			url: "/usermanage/checkadminzzjgdm.html",
			data: {"zzjgdm": dm, "usertype": '${cust.usertype}', "f_id": '${cust.f_id}'},
			dataType:"json",
			success: function(data){
				if (data.msg != "succ") {
					if (confirm(data.msg)) {
						$.ajax({
							type: 'post',
							url: "/usermanage/releqiye.html",
							data: {"pk_qiyeid": data.pk_qiyeid, "custid": '${cust.id}'},
							dataType:"json",
