...
<%-- 		<s:hidden name="type"/> --%>
		<s:hidden name="activities.type"/>
		<input type="hidden" value="<s:property value="activities.catalogID"/>" id="catalogID"/>
		<table class="table table-bordered">
			...
			<tr style="display: none;">
				<td>id</td>
				<td colspan="3"><s:hidden name="activities.id" label="id"  /></td>
			</tr>
				...
				<s:textfield name="activities.activities_name" id="activities_name" size="64" readonly="false" data-rule="名称:required;activities_name;length[2~128]"/>
				</td>
				</tr>
				...
				<td style="text-align: left;"  colspan="3">
				<s:textfield name="activities.activities_theme" id="activities_theme" size="80" readonly="false" data-rule="主题:required;activities_theme;length[2~128]"/>
				</td>
				</tr>
 				 -->
 								<tr>
				...
				<td style="text-align: left;" colspan="3">
				<s:textfield name="activities.address" id="address"   size="80" readonly="false" data-rule="活动地址:required;address;length[2~64]"/>
				</td>
				 </tr>
				...
				<td style="text-align: left;" colspan="3">
				<s:textfield name="activities.agenciesname" id="agenciesname"   size="80" readonly="false" data-rule="主办方:required;agenciesname;length[2~64]"/>
				</td>
				</tr>
				...
				<td style="text-align: left;" colspan="3">
 				<s:checkboxlist name="activities.adapttype" id="adapttype" list="%{#application.dic.khlx.childs}" 
 				listKey="id" listValue="name" value="#request.activities.adapttype.split(',')"/>
				</td>
				</tr>
				...
				<td style="text-align: left;" colspan="3" >
					...
					<s:select list="%{#application.dic.activitytype.childs}" listKey="id"  headerKey="" headerValue=""  listValue="name" name="activities.activitiestype" label="活动类型"      />  
           	 	</td>
           	 	
           	 	
           	 	<td style="text-align: right;display: none;">分站</td>
           		<td style="text-align: left;display: none;">
                   	<select name="activities.substation" id="substation">
				...
				<input type="text" class="Wdate" id="begintime"  value="<s:property value="activities.begintime"/>" name="activities.begintime" onfocus="WdatePicker({skin:'whyGreen',dateFmt: 'yyyy-MM-dd HH:mm:ss'  })"  />
				
				</td>
			 
				<th class="td_right">结束时间</th>
				<td style="text-align: left;">
				<input type="text" class="Wdate" id="endtime"   value="<s:property value="activities.endtime"/>"  name="activities.endtime" onfocus="WdatePicker({skin:'whyGreen',dateFmt: 'yyyy-MM-dd HH:mm:ss'  })"  />
				</td>
				</tr>
				...
				<td style="text-align: left;">
				<input type="text" class="Wdate" id="stoptime"  value="<s:property value="activities.stoptime"/>"   name="activities.stoptime" onfocus="WdatePicker({skin:'whyGreen',dateFmt: 'yyyy-MM-dd HH:mm:ss'  })"  />
				...
				<td style="text-align: left;">
				<s:textfield name="activities.maxattend" id="maxattend" readonly="false" data-rule="最大参加人数:required;maxattend;length[1~4]"/>
				</td>
				</tr>
 					<tr>
					<th class="td_right">嘉宾</th>
					<td style="text-align: left;">
					<s:textfield name="activities.activities_guests" id="activities_guests" readonly="false" data-rule="嘉宾:activities_guests;length[2~18]"/>
					</td>
					<th></th>
					<td></td>
					...
					<td style="text-align: left;">
						<s:textfield name="activities.lnglat" id="lnglat" readonly="false" data-rule="经纬度:lnglat;length[0~128]"/>
					</td> --%>
				</tr>
				<tr>
					<td  style="text-align: left;"   >
					<input type="button" name="insertfile"  id="insertfile" value="附件" class="btn btn-success"/>（非必填）
						</td>
						<td style="text-align: left;"  colspan="3">
							 <input type="text" name="fileurl" value="${fileurl }" id="fileurl" style="width: 600px;">
					</td>
			...
			<tr>
					...
					<input type="button" name="filemanager"  id="filemanager" value="封面图片（304px*203px）" class="btn btn-success"/>
						...
						<td style="text-align: left;"  colspan="3">
									<s:textfield type="text" id="activities_image" name="activities.activities_image"   style="width: 600px;"  />
						<s:if test="activities.activities_image!=null">
							<img src="${news.cover }" class="smallpic"/>
                            <a target="_blank" href="<s:property value="activities.activities_image" />">
                                <img style="max-width: 50px;max-height: 50px;" src="<s:property value='activities.activities_image'/>">
                            </a>
                            </s:if>		 
					</td>
			...
			<tr>
				<th class="td_right">内容
					<br>
					<span>*建议先用word文档撰写内容后，复制粘贴到此处</span>
				</th>
				<td style="text-align: left;"  colspan="3">
				<s:textarea name="activities.activities_detail"  rows="3" cols="80" style="width:100%;height:400px;visibility:hidden;"   id="activities_detail" readonly="false"  />
				</td>
				</tr>		
		 <tr>
		 <tr>
                ...
                <td style="text-align: left;" colspan="3"><s:select
                        list="%{#application.dic.datasources.childs}" listKey="id" listValue="name"  headerKey="" headerValue=""
						name="activities.data_source" id="data_source" />
                </td>
         </tr>
         <%-- <tr>
                <th class="td_right">app编号</th>
                <td style="text-align: left;" >
                <s:textfield name="activities.appid" id="appid" readonly="ture" />
                 <c:if test="${activities.appid eq '' or empty activities.appid }">
                        <s:a href="javascript:getAppList()" cssClass="btn btn-success" id="selectPbczdbase">请选择</s:a>
                    </c:if>
                </td>
                <th class="td_right">用户id</th>
                <td style="text-align: left;" >
                <s:textfield name="activities.releuserid" id="releuserid" readonly="ture" />
                </td>
         </tr> --%>
         <tr>
         
				<td colspan="4" style="text-align: center;">
						<button method="/usermanage/actiUpdate.html" class="btn btn-success">
...
<script>
	var editor;
	KindEditor.ready(function(K) {
		editor = K.create('textarea[name="activities.activities_detail"]', {
			allowFileManager : true
		});
