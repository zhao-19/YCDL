		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
                    ...
                    html += '	            <div class="guanbi" onclick="guanbi()">×</div>';
...
//						 {"error":0,"url":"http://10.0.254.4:8080/interestapp/interestapp/20160627185249_920.jpg"}
						$("#uploadProgress_" + file.index).hide();
						$("#uploadSuccess_" + file.index).show();
						//alert(response);
						var ypic =eval('('+response+')').url;
						var picnm = $("#picnum").val();
						if(picnm.length>0){
							$("#file").hide();
							$("#zz").hide();
							$("#preview").html("");
						}
						$("#"+picnm).html("<img src="+ypic+">");
						$("input[name="+picnm+"]").val(ypic);
