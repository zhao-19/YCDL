	...
	<div class="list fr">
		<div class="rightbox">
			<div id="zaiquanxinxi-update" class="vipinfo" >
				<div class="title"><li><span class="t">小额贷款数据报送</span><a href="/userextend/getXdinfo.html?t=a" id="addA"><span class="update">添加</span></a></li>
				</div>
				<table id="ycacti" align="center" border="0" cellpadding="0" cellspacing="1">
					...
					<tbody>
 					<s:iterator value="tbjrxdList">
						<tr>
			                <td>&nbsp;${time_port }</td>
			                <td>&nbsp;${loan_cu_amt }</td>
			                <td>&nbsp;${loan_count_amt }</td>
			                <td>&nbsp;${loan_count }</td>
			                <td>&nbsp;
			                ...
			                <td>
				                <input type="button" value="查看" class="an ck op opr" onclick="$('#hideA').attr('href','/userextend/getXdinfo.html?t=v&queryid=${id}')[0].click();"/>
				                <s:if test="status=='aaaaaaaaaaaaaaaaaaaaaaaaaaaaa080'"><input type="button" value="修改" class="an xg op opl" onclick="$('#hideA').attr('href','/userextend/getXdinfo.html?queryid=${id}')[0].click();"/></s:if>
			                </td>
						</tr>
					</s:iterator>
					</tbody>
