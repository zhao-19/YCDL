	...
	<div class="list fr">
		<div class="rightbox">
 			<div id="peixunhuodong" class="vipinfo" >
				...
				<span class="t">贴息申报信息</span>
				<span class="daochutishi daochutishi2"></span>
...
<% if(SystemManager.SBLIST!=null) {
				...
				<s:if test="#request.isBase == 1">
		                    <a href="/interestapp/toAdd.html?type=${type}">
		                    	<span id="CreateTBpxhd" class="update">申报补贴</span>
		                    </a>
	                    </s:if>
	                    <s:else>
		                    <a href="javascript:void(0);" onclick='return myConfirm("你还为填写企业基本情况表，需先填写后才可申报！","立即填报","取消","/interestapp/toAdd.html?type=${type}");'>
			                    	<span id="CreateTBpxhd" class="update">申报补贴</span>
		                    </a>
	                    </s:else>
				</s:else>
				...
				<table id="ycacti" align="center" border="0" cellpadding="0" cellspacing="1">
					...
					<s:iterator value="#request.pager.list">
	               	 ...
	               	 <td><s:date name="lxjsksrq"  format="yyyy-MM-dd"/>  到 <s:date name="lxjsjzrq"  format="yyyy-MM-dd"/> </td>
						...
						<td>
                        	<input class="an xg op  opl" lang="<s:property value="id"/>" type="button" value="" class="pxhdjs">
                        	<input class="an ck op opr" type="button" value="" lang="<s:property value="id"/>">
                        	<%-- <input class="an xzp op oprd"  type="button" value="" onclick="location.href='interestapp/exportPDF.html?id=<s:property value="id"/>'" title="导出申报资料"  lang="<s:property value="id"/>"  >--%>
                        	<input class="an xzw op oprd"  type="button" value="" msg="txdb" title="导出申报资料"  lang="<s:property value="id"/>"  >
				...
				opls.click(function(){
						...
						location.href="/interestapp/tostep1.html?id="+id+"&t=u";
				});
				//查看
				oprs.click(function(){

					var id=$(this).attr("lang");
					location.href="/interestapp/tostep1.html?id="+id+"&t=v";
				});
				...
				//导出申报资料
 				...
 				var beginDate="2017-04-01"; //这之前的数据都不能导出 
				var oprds = $("#ycacti .oprd");				
				oprds.each(function(){
					var stateinput= $(this).parent().prev().html();
					
					var endDate=$(this).parent().prev().prev().html();  
					var d1 = new Date(beginDate.replace(/\-/g, "\/"));  
				 	var d2 = new Date(endDate.replace(/\-/g, "\/"));  
				 
				 
					if(stateinput != "审批通过" || (d2<d1)){
						$(this).attr("disabled","true");
 ...
 <%@ include file="../sbapp/tanchudownload.jsp" %>
</body>
</html>
